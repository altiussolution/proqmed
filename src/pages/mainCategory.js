    ...
    return <div id="products" className="row list-group catgoryPage">
      {
        mainCategory3.map((el, index) => (
         <> {el.product_count != 0 && el.node_data == true && <div className="item product_item"> {el.product_count != 0 && <Link to={getCategoryURL(el)}
            key={index} >
            <div className="thumbnail">
              <div className="product_img">
              {el.product_count != 0 && el.node_data == true &&   <img className="img-fluid" src={`${el.image}`} onError={e => (e.target.src = ImageNotFound)}  /> }
              </div>
              <div className="caption">
              {el.product_count != 0 && el.node_data == true && <p className="product_text">
                {el.name}
                </p>}
              </div>
            </div>
            <ul className="sub_categoriesList">
              {
                el.children_data.map((e, index) => (
                  <li key={e.name}>{e.product_count != 0 &&  e.node_data == true &&<Link className="sub_categoriesItem" to={getCategoryURL(e)}>{e.name}</Link>}</li> 
                ))
              }
            </ul>
          </Link> }</div>}</>
