      ...
      for (let prop in product) {
        if (prop !== "items" && prop !=="Special Price") {
          displayFilters[prop] = displayFilters[prop]
            ? displayFilters[prop].add(prop=="Todays Deal" ? product[prop][localStorage.group ? localStorage.group :0]:product[prop])
            : new Set([prop=="Todays Deal" ? product[prop][localStorage.group ? localStorage.group :0]:product[prop]]);
      ...
      for (var i = minValue; i <= maxValue; i++) {
        ...
        var result = products.filter(function (prod) {
          return numbers.some(function (o2) {
              return Math.round(prod[0].items.prices[localStorage.group ? localStorage.group :0]['final_price']) === o2; // return the ones with equal id
         });
      });
      let arr1=[];
      result.forEach(el => {
  ...
  const filterOperation = products => {
    ...
    Object.keys(filters).forEach((prop, index) => {
      ...
      products.forEach(el => {
        if (index === 0) {
          const product = convertToObject(el.flat());
          if (product[prop] && filters[prop].includes(prop=="Todays Deal" ? product[prop][localStorage.group ? localStorage.group :0]:product[prop])) {
          ...
          tempFilteredProducts = tempFilteredProducts.filter(product => {
            return product[prop] && filters[prop].includes(prop=="Todays Deal" ? product[prop][localStorage.group ? localStorage.group :0]:product[prop]);
          });
        }  
      });
    });
  ...
  const filterOperationPrice = async(products,numbers) => {
   ...
   if(checked){
     var res = filterCheckBox.filter(function (prod){
      return numbers.some(function (o2) {     
        return Math.round(prod.items.prices[localStorage.group ? localStorage.group :0]['final_price']) === o2; // return the ones with equal id
            ...
            {values.map((val, i) => (
             ...
             {val != "" &&  <div
              ...
              /> 
                  ...
                  <span >{key == "Todays Deal" ? (val==9 ? Math.round(val) :Math.round(val) ) : (key == "Rating" && val!="") ? val  : key == "Special Price" ? Math.round(val) : val}
                 <span className={key == "Rating" ? "fa fa-star" : "inp_val"}>{key == "Todays Deal" ? (val==9 ? "% & below" :"% & more") : (key == "Rating" && val!="") ? "& above" : ""}</span>
