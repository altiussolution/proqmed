            ...
            list.map((el, index) => (
              el.map(item => (
                <figure key={item.id} className="itm_list">
                  {item.product_count != 0 && <Link to={getCategoryURL(item)} state={item} className="itm_list_title">{item.name}</Link>}
                  {
                    item.children_data.map(grand_child => (
                      <span key={grand_child.id} >{grand_child.product_count != 0 && <Link to={getCategoryURL(grand_child)} state={grand_child}>{grand_child.name}</Link>}</span> 
                    ))
                  }
